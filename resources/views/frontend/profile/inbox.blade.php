...
<div class="container mt-4 mb-4 pt-5">
            ...
            <li class="list-group-item list-group-item-action d-flex align-items-center border rounded mb-3 px-2 position-relative"
                ...
                <div class="d-flex w-100 align-items-center gap-md-2">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <p class="text-muted mb-0 text-truncate message-content">{{ $conversation->message }}</p>
                    </div>
                    <div class="d-flex flex-column text-end ps-2">
                        <small class="text-muted mb-1 text-nowrap pe-md-1">{{ $conversation->created_at->diffForHumans() }}</small>
                        <button class="btn w-100 h-100 py-md-2 px-md-5"
                            ...
                            style="background-color: #0064A7; color: white;" onclick="openChat(this)">
    ...
    <li class="list-group-item border rounded mb-3 p-3 bg-white">
        <div class="d-flex align-items-start gap-3">

            {{-- Grey round dot in front of image --}}
            <span style="
                width: 17px; 
                height: 17px; 
                background-color: #9D9999; /* full grey */
                border-radius: 50%; /* round */
                display: inline-block;
                flex-shrink: 0;
                margin-right: 8px; /* spacing before the image */
                margin-top: 12px; /* vertically align roughly center with image */
            "></span>

            {{-- Jyotish Photo --}}
            <img src="{{ $adminMessage->jyotish && $adminMessage->jyotish->photo ? asset('storage/' . $adminMessage->jyotish->photo) : asset('path/to/default-jyotish.png') }}"
                alt="Jyotish Photo"
                class="rounded-circle"
                style="width: 40px; height: 40px; object-fit: cover;">

            {{-- Jyotish Name + Title + Description --}}
            <div class="flex-grow-1 w-100">
                <div class="d-flex justify-content-between align-items-center mb-1">
                    <p class="mb-0 fw-bold">
                        {{ $adminMessage->jyotish->name ?? 'Unknown Jyotish' }}
                        <small class="text-primary">(Astrologist)</small>
                    </p>
                    <small class="text-muted text-nowrap">{{ $adminMessage->created_at->format('h:i A') }}</small>
                </div>

                <p class="mb-1 text-muted">{!! $adminMessage->title !!}</p>
                <p class="mb-0 text-muted">{!! $adminMessage->description !!}</p>

                {{-- Lock Icon + Replies Disabled Below Description --}}
                <div>
                    <small class="text-muted d-flex align-items-center gap-1 mt-1" style="font-size: 13px;">
                        <i class="bi bi-lock-fill"></i>
                        This is one-way message - replies are disabled
                    </small>
                </div>
            </div>
...
@endforeach
    ...
    <script>
        function toggleDetails(id) {
            const el = document.getElementById('adminMessageDetails' + id);
