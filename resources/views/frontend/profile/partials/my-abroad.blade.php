                    ...
                    <div class="flex">
                        ...
                        <div class="d-flex gap-1">
                            <button class="edit mb-2 p-1 px-2"
                                data-productTitle="{{$aboard->productTitle}}" data-pricing=" {{$aboard->pricing}} " data-description=" {{$aboard->productDescription}} "
                                data-category="{{ $aboard->category }}" data-country="{{$aboard->country}}" data-url="{{route('aboards.update',$aboard->id)}}"
                                data-img="{{ $aboard->productThumbnail }}" onclick="openUpdateForm(this)">Edit</button>

                            <!-- Delete Button -->
                            <form action="{{ route('aboards.destroy', $aboard->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="delete mb-2 p-1 px-2" onclick="return confirm('Are you sure you want to delete this product?')">Delete</button>
                            </form>
                        </div>
                    </div>
                </div>
            ...
            <form id="editItemForm" action="#" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3 form-floating">
                    <select class="form-select abroad-deal-1 fw-semibold" id="newCountrySelect" name="country" required>
                        <option value="" disabled {{ $aboard->country ? '' : 'selected' }}>Choose a Country</option>
                ...
                <!-- Category Select -->
                <div class="mb-3 form-floating">
                    <select class="form-select abroad-deal-1 fw-semibold" id="categorySelect" name="productCategoryId" required>
                        <option value="" disabled selected>Choose a Category</option>
                        @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ $aboard->productCategoryId == $category->id ? 'selected' : '' }}>
                ...
                <div class="mb-3 form-floating abroad-deal-1 fw-semibold">
                    <input type="text" class="form-control abroad-deal-1 fw-semibold" id="titleInput" name="productTitle"
                        placeholder="Title" value="{{ old('productTitle', $aboard->productTitle) }}" required>
                ...
                <div class="mb-3 form-floating abroad-deal-1 fw-semibold">
                    <textarea class="form-control abroad-deal-1 fw-semibold" id="descriptionInput" name="productDescription"
                        rows="4" placeholder="Describe..." required>{{ old('productDescription', $aboard->productDescription) }}</textarea>
                ...
                <div class="mb-3 input-group">
                    <input type="text" class="form-control abroad-deal-1 fw-semibold border-0" placeholder="Add to your Post" id="addToPostInput">
                    <button class="btn abroad-deal-1 fw-semibold border-0" style="border-top-right-radius: 5px; border-bottom-right-radius: 5px;" type="button" id="uploadImageButton">
...
<script>
    ...
    const openUpdateForm = (e) => {
        ...
        updateForm.querySelector('[name="productDescription"]').innerHTML = e.getAttribute('data-productDescription')
