                ...
                @foreach ($images as $image)
                    <div class="col-md-4 my-1 flex-1">
                        <div class="image-container rounded overflow-hidden position-relative">
                            <img src="{{ asset($image->image_path) }}" alt="BoardingPass" class="img-fluid" data-bs-toggle="modal" data-bs-width="100%" data-bs-target="#imagePreviewModal" data-image="{{ asset($image->image_path) }}" style="cursor: pointer;">
                            <button type="button"
                                class="delete-image-btn position-absolute top-0 start-0 p-0 text-center text-danger rounded rounded-circle btn"
