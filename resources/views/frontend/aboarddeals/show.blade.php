        ...
        <div class="container mt">
            <div class="row mt">
                <div class="col-sm-12 col-md-3 col-lg-3 border border-1 rounded p-2 fixed-height">
                    <!-- Responsive Image -->
                    <img src="{{$aboard->productThumbnail ? asset($aboard->productThumbnail) : asset('Images/default-image.png') }}"
                    ...
                    <div class="profile-price d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            <img src="{{ asset('storage/' . $aboard->jobSeeker->userThumbnail[0]) }}"
                                class="rounded-circle abroad-chat" alt="Profile picture of Ram Baral">
                            <div class="ms-2">
                                <p class="fw-semibold mb-0">{{$aboard->jobSeeker->firstName}} {{$aboard->jobSeeker->lastName}}</p>
                                <p class="text-muted mb-0 mt-0">{{$aboard->phoneNumber}}</p>
                            </div>
                        </div>
                        <h3 class="mt-2 mt-md-0 text-end price-text">{{$aboard->pricing}}</h3>
                    </div>
                    ...
                    <div class="mt-3 mb-3 d-flex flex-wrap justify-content-center gap-2">
                        ...
                        <button class="btn custom-outline-btn flex-grow-1"
                            data-user-id="{{ $aboard->jobSeekerId }}"
                    ...
                    <div id="copyMessage" class="alert alert-success alert-dismissible fade show" role="alert" style="display: none;">
                ...
                <div class="col-md-9 ps-md-4 mt-3 mt-md-0 border-0 fixed-height">
                    <h2 class="fw-semibold">{{$aboard->productTitle}}</h2>
                    ...
                    <div class="row mt-3 mb-3" id="comment" style="display: none;">
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <form action="{{route('aboardcomment.destroy',$cmt->id)}}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                        ...
                        <form action="{{ route('aboardcomment.store') }}" method="post">
                            @csrf
                            <input type="hidden" value="{{$aboard->id}}" name="productId">
            ...
            <div class="row g-2 mt-0" id="product-list">
                ...
                <div class="col-lg-3 col-md-3 col-sm-6 col-12 product" data-category="electronics">
                    <div class="card-bdy-packages">
                        <a href="{{route('aboards.show',$product->id)}}" class="text-decoration-none text-black">
                ...
                @if ( $similarProducts->hasMorePages() || $similarProducts->currentPage() !=1)

                <div class="row mt-3">
                    <nav>
                        <ul class="pagination justify-content-end converter">
                            {{-- Previous Button --}}
                            @if ( $similarProducts->onFirstPage())
                            ...
                            <li class="page-item">
                                <a class="page-link primary_color_text"
                                    href="{{  $similarProducts->previousPageUrl() }}">&lt;</a>
                            </li>
                            @endif

                            {{-- Pagination Numbers --}}
                            @php
                            $currentPage = $similarProducts->currentPage();
                            $lastPage = $similarProducts->lastPage();
                            ...
                            <li class="page-item">
                                <a class="page-link primary_color_text" href="{{  $similarProducts->url(1) }}">1</a>
                            </li>
                            ...
                            @for ($i = max(1, $currentPage - $pageRange); $i < $currentPage; $i++)
                                <li class="page-item">
                                <a class="page-link primary_color_text"
                                    href="{{  $similarProducts->url($i) }}">{{ $i }}</a>
                                </li>
                                @endfor
                                ...
                                @for ($i = $currentPage + 1; $i <= min($lastPage, $currentPage + $pageRange); $i++)
                                    ...
                                    @if ($currentPage < $lastPage - $pageRange)
                                        ...
                                        <li class="page-item">
                                            <a class="page-link primary_color_text"
                                                href="{{  $similarProducts->url($lastPage) }}">{{ $lastPage }}</a>
                                        </li>
                                        @endif

                                        {{-- Next Button --}}
                                        @if ( $similarProducts->hasMorePages())
                                        <li class="page-item">
                                            <a class="page-link primary_color_text"
                                                href="{{  $similarProducts->nextPageUrl() }}">&gt;</a>
                                        </li>
                                        @else
                                        <li class="page-item disabled">
                                            <a class="page-link primary_color_text">&gt;</a>
