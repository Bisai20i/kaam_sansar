...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="content-wrapper">
            <h4 class="fw-bold mb-4"><span class="text-muted fw-light"> </span>Visa Type Category</h4>
                ...
                <div class="col-12 col-lg-8">
                    <div class="card mb-4">
                        <div class="card-header d-flex align-items-center justify-content-between">
                            <h5 class="mb-0">{{ isset($visaType) ? 'Edit' : 'Add' }} Visa Type Category Lists</h5>
                        </div>
                        <div class="card-body">
                            <form method="POST"
                                action="{{ isset($visaType)
                                    ? route('VisaTypeList.update', ['visaType' => $visaTypesList->id])
                                    : route('VisaTypeList.store') }}"
                                id="JobForm">
                                @csrf
                                @if (isset($visaType))
                                    @method('PUT')
                                ...
                                <input type="hidden" name="visa_types_lists" id="jobCategories">

                                <div class="mb-3">
                                    <label for="subTask" class="form-label">
                                        Visa Type Name <span class="text-danger">*</span>
                                    </label>
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="subTask"
                                            placeholder="Add Visa Type" style="flex: 2;">
                ...
                <div class="col-12 col-md-12">
                    <div class="card mb-4">
                        <div class="card-header d-flex align-items-center justify-content-between">
                            ...
                            <small class="text-muted float-end">List of Countries</small>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($visaTypesLists as $visaTypesList)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $visaTypesList->visaTypeName }}</td>
                                                <td>
                                                    <p
                                                        class="text-capitalize  badge {{ $visaTypesList->publishStatus == '1' ? 'bg-success' : 'bg-danger' }} m-2">
                                                        {{ $visaTypesList->publishStatus == '1' ? 'Published' : 'Unpublished' }}
                                                    </p>
                                                </td>
                                                <td>
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu">
                                                            ...
                                                            <a class="dropdown-item text-{{ $visaTypesList->publishStatus == '1' ? 'danger' : 'success' }}"
                                                                href="javascript:void(0);" data-bs-toggle="modal"
                                                                data-bs-target="#publishUnpublishModal"
                                                                onclick="setPublishUnpublishFormAction({{ $visaTypesList->id }}, '{{ $visaTypesList->publishStatus }}', '{{ $visaTypesList->visaTypesListName }}')">
                                                                <i
                                                                    class="bx bx-{{ $visaTypesList->publishStatus == '1' ? 'x' : 'check' }} me-1"></i>
                                                                {{ $visaTypesList->publishStatus == '1' ? 'Unpublish' : 'Publish' }}
                                                            </a>

                                                            <!-- Edit Trigger -->
                                                            <a class="dropdown-item text-primary" href="#"
                                                                data-bs-toggle="modal"
                                                                data-bs-target="#editvisaTypesListModal{{ $visaTypesList->id }}">
                                                                <i class="bx bx-edit me-1"></i> Edit
                                                            </a>
                                                            <!-- Delete Trigger -->
                                                            <a class="dropdown-item text-danger" href="javascript:void(0);"
                                                                data-bs-toggle="modal" data-bs-target="#deleteModal"
                                                                onclick="setDeleteFormAction({{ $visaTypesList->id }})">
                                            ...
                                            <div class="modal fade" id="editvisaTypesListModal{{ $visaTypesList->id }}"
                                                tabindex="-1" aria-labelledby="editvisaTypesListModalLabel"
                                                aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <form id="editvisaTypesListForm"
                                                            action="{{ route('VisaTypeList.update', ['VisaTypeList' => $visaTypesList->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('PUT')
                                                            <div class="modal-header">
                                                                <h5 class="modal-title" id="editvisaTypesListModalLabel">
                                                                    Edit Visa Type</h5>
                                                            ...
                                                            <div class="modal-body">
                                                                <div class="mb-3">
                                                                    <label for="editvisaTypesListName" class="form-label">
                                                                        Visa Type Name</label>
                                                                    <input type="text" class="form-control"
                                                                        id="editvisaTypesListName" name="name"
                                                                        value="{{ $visaTypesList->visaTypeName }}"
                                                                        required>
                                                                </div>
                                                                <div class="mb-3">
                                                                    <label for="editvisaTypesListStatus"
                                                                        class="form-label">Publish Status</label>
                                                                    <select class="form-select"
                                                                        id="editvisaTypesListStatus" name="status"
                                                                        required>
                                                                        <option value="1"
                                                                            {{ $visaTypesList->publishStatus == '1' ? 'selected' : '' }}>
                                                                            Published</option>
                                                                        <option value="0"
                                                                            {{ $visaTypesList->publishStatus == '0' ? 'selected' : '' }}>
                                                                            Unpublished</option>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="publishUnpublishModalLabel">
                        Change Visa Type Publish Status to <strong id="modalTitle"></strong> Status
                    </h5>
    ...
    <script>
        ...
        function setDeleteFormAction(id) {
            document.getElementById('deleteForm').action = '{{ route('VisaTypeList.destroy', '') }}/' + id;
        }
    </script>
    ...
    <script>
        ...
        const publishUrl = @json(route('VisaTypeList.publish', ['id' => '__ID__']));
        const unpublishUrl = @json(route('VisaTypeList.unpublish', ['id' => '__ID__']));

        // Function to dynamically update modal content
        function setPublishUnpublishFormAction(visaTypesListId, currentStatus) {
            ...
            if (currentStatus === '1') {
                ...
                modalMessage.textContent = 'Are you sure you want to unpublish this visa type?';
                actionButton.classList.remove('btn-success');
                actionButton.classList.add('btn-warning');
                buttonText.textContent = 'Unpublish';
                publishUnpublishForm.action = unpublishUrl.replace('__ID__', visaTypesListId);
            } else {
                // Set content for publishing
                modalTitle.textContent = 'Published';
                modalMessage.textContent = 'Are you sure you want to publish this visa type?';
                actionButton.classList.remove('btn-warning');
                actionButton.classList.add('btn-success');
                buttonText.textContent = 'Publish';
                publishUnpublishForm.action = publishUrl.replace('__ID__', visaTypesListId);
            }
        }
