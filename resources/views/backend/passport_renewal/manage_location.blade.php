...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="content-wrapper">
            <h4 class="fw-bold mb-4"><a href="{{ route('passport.renewal') }}" class="text-muted fw-semibold">Passport Renewals /</a> 
                ...
                <a href="{{ route('passportDistrictList.index', ['provience_id' => $provience->id]) }}" class="text-muted fw-semibold">{{ ucfirst($district->districtName) }} /</a>
                 Locations</h4>
            <!-- Main Content -->
            <div class="row">
                ...
                <div class="col-12 col-lg-8">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <form method="POST"
                                action="{{ route('passportLocationList.store') }}"
                                id="JobForm">
                                @csrf
                               

                                <!-- Hidden input to store the job categories -->
                                <input type="hidden" name="location_lists" id="jobCategories">
                                <input type="hidden" name="district_id" value="{{ $district->id }}" id="provience_id">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="subTask"
                                            placeholder="Add Location Name" style="flex: 2;">
                        ...
                        <div class="card-body">
                            <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($passportLocationLists as $passportLocationList)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $passportLocationList->locationName }}</td>
                                                <td>
                                                    <p
                                                        class="text-capitalize  badge {{ $passportLocationList->publishStatus == '1' ? 'bg-success' : 'bg-danger' }} m-2">
                                                        {{ $passportLocationList->publishStatus == '1' ? 'Published' : 'Unpublished' }}
                                                    </p>
                                                </td>
                                                <td>
                                                    <a href="{{ route('passportDateTime.index', ['location_id' => $passportLocationList->id]) }}"
                                                        class="btn btn-info btn-sm  text-white">Manage Date and Time</a>
                                                </td>
                                                <td>
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu">
                                                            ...
                                                            <a class="dropdown-item text-{{ $passportLocationList->publishStatus == '1' ? 'danger' : 'success' }}"
                                                                href="javascript:void(0);" data-bs-toggle="modal"
                                                                data-bs-target="#publishUnpublishModal"
                                                                onclick="setPublishUnpublishFormAction({{ $passportLocationList->id }}, '{{ $passportLocationList->publishStatus }}')">
                                                                <i
                                                                    class="bx bx-{{ $passportLocationList->publishStatus == '1' ? 'x' : 'check' }} me-1"></i>
                                                                {{ $passportLocationList->publishStatus == '1' ? 'Unpublish' : 'Publish' }}
                                                            </a>

                                                            <!-- Edit Trigger -->
                                                            <a class="dropdown-item text-primary" href="#"
                                                                data-bs-toggle="modal"
                                                                data-bs-target="#editvisaCountryListModal{{ $passportLocationList->id }}">
                                                                <i class="bx bx-edit me-1"></i> Edit
                                                            </a>
                                                            <!-- Delete Trigger -->
                                                            <a class="dropdown-item text-danger" href="javascript:void(0);"
                                                                data-bs-toggle="modal" data-bs-target="#deleteModal"
                                                                onclick="setDeleteFormAction({{ $passportLocationList->id }})">
                                            ...
                                            <div class="modal fade" id="editvisaCountryListModal{{ $passportLocationList->id }}"
                                                tabindex="-1" aria-labelledby="editvisaCountryListModalLabel"
                                                aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <form id="editvisaCountryListForm"
                                                            action="{{ route('passportLocationList.update', ['passportLocationList' => $passportLocationList->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('PUT')
                                                            ...
                                                            <div class="modal-body">
                                                                <div class="mb-3">
                                                                    ...
                                                                    <input type="text" class="form-control"
                                                                        ...
                                                                        value="{{ $passportLocationList->locationName }}"
                                                                        required>
                                                                </div>
                                                                <div class="mb-3">
                                                                    ...
                                                                    <select class="form-select"
                                                                        ...
                                                                        required>
                                                                        <option value="1"
                                                                            {{ $passportLocationList->publishStatus == '1' ? 'selected' : '' }}>
                                                                            Published</option>
                                                                        <option value="0"
                                                                            {{ $passportLocationList->publishStatus == '0' ? 'selected' : '' }}>
                                                                            Unpublished</option>
    ...
    <script>
        ...
        function setDeleteFormAction(id) {
            document.getElementById('deleteForm').action = '{{ route('passportLocationList.destroy', '') }}/' + id;
        }
    </script>
    ...
    <script>
        ...
        const publishUrl = @json(route('passportLocationList.publish', ['id' => '__ID__']));
        const unpublishUrl = @json(route('passportLocationList.unpublish', ['id' => '__ID__']));
