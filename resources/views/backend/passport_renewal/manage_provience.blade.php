...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="content-wrapper">
            <h4 class="fw-bold mb-4"><a href="{{ route('passport.renewal') }}" class="text-muted fw-semibold">Passport Renewals /</a> <a href="{{ route('passportCountryList.index') }}" class="text-muted fw-semibold">{{ ucfirst($country->countryName) }} /</a> Proviences</h4>
            <!-- Main Content -->
            <div class="row">
                ...
                <div class="col-12 col-lg-8">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <form method="POST"
                                action="{{ isset($passportProvienceList)
                                    ? route('passportProvienceList.update', ['passportProvienceList' => $passportProvienceList->id])
                                    : route('passportProvienceList.store') }}"
                                id="JobForm">
                                @csrf
                                @if (isset($passportProvienceList))
                                    @method('PUT')
                                @endif

                                <!-- Hidden input to store the job categories -->
                                <input type="hidden" name="provience_lists" id="jobCategories">
                                <input type="hidden" name="country_id" value="{{ $country->id }}" id="country_id">

                                <div class="mb-3">
                                    <label for="subTask" class="form-label">
                                        Provience Name <span class="text-danger">*</span>
                                    </label>
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="subTask"
                                            placeholder="Add Provience Name" style="flex: 2;">
                ...
                <div class="col-12 col-md-12">
                    <div class="card mb-4">
                        <div class="card-header d-flex align-items-center justify-content-between">
                            <h5 class="mb-0">Proviences</h5>
                            <small class="text-muted float-end">List of Proviences</small>
                        </div>
                        <div class="card-body">
                                ...
                                <table class="table table-bordered">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Provience Name</th>
                                            <th>Publish Status</th>
                                            <th>Manage Districts</th>
                                            <th>Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($passportProvienceLists as $passportProvienceList)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $passportProvienceList->provienceName }}</td>
                                                <td>
                                                    <p
                                                        class="text-capitalize  badge {{ $passportProvienceList->publishStatus == '1' ? 'bg-success' : 'bg-danger' }} m-2">
                                                        {{ $passportProvienceList->publishStatus == '1' ? 'Published' : 'Unpublished' }}
                                                    </p>
                                                </td>
                                                <td>
                                                    <a href="{{ route('passportDistrictList.index', ['provience_id' => $passportProvienceList->id]) }}"
                                                        class="btn btn-info btn-sm  text-white">Manage Districts</a>
                                                </td>
                                                <td>
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu">
                                                            ...
                                                            <a class="dropdown-item text-{{ $passportProvienceList->publishStatus == '1' ? 'danger' : 'success' }}"
                                                                href="javascript:void(0);" data-bs-toggle="modal"
                                                                data-bs-target="#publishUnpublishModal"
                                                                onclick="setPublishUnpublishFormAction({{ $passportProvienceList->id }}, '{{ $passportProvienceList->publishStatus }}')">
                                                                <i
                                                                    class="bx bx-{{ $passportProvienceList->publishStatus == '1' ? 'x' : 'check' }} me-1"></i>
                                                                {{ $passportProvienceList->publishStatus == '1' ? 'Unpublish' : 'Publish' }}
                                                            </a>

                                                            <!-- Edit Trigger -->
                                                            <a class="dropdown-item text-primary" href="#"
                                                                data-bs-toggle="modal"
                                                                data-bs-target="#editvisaCountryListModal{{ $passportProvienceList->id }}">
                                                                <i class="bx bx-edit me-1"></i> Edit
                                                            </a>
                                                            <!-- Delete Trigger -->
                                                            <a class="dropdown-item text-danger" href="javascript:void(0);"
                                                                data-bs-toggle="modal" data-bs-target="#deleteModal"
                                                                onclick="setDeleteFormAction({{ $passportProvienceList->id }})">
                                            ...
                                            <div class="modal fade" id="editvisaCountryListModal{{ $passportProvienceList->id }}"
                                                tabindex="-1" aria-labelledby="editvisaCountryListModalLabel"
                                                aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <form id="editvisaCountryListForm"
                                                            action="{{ route('passportProvienceList.update', ['passportProvienceList' => $passportProvienceList->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('PUT')
                                                            ...
                                                            <div class="modal-body">
                                                                <div class="mb-3">
                                                                    <label for="editvisaCountryListName"
                                                                        class="form-label">Provience Name</label>
                                                                    <input type="text" class="form-control"
                                                                        id="editvisaCountryListName" name="name"
                                                                        value="{{ $passportProvienceList->provienceName }}"
                                                                        required>
                                                                </div>
                                                                <div class="mb-3">
                                                                    ...
                                                                    <select class="form-select"
                                                                        ...
                                                                        required>
                                                                        <option value="1"
                                                                            {{ $passportProvienceList->publishStatus == '1' ? 'selected' : '' }}>
                                                                            Published</option>
                                                                        <option value="0"
                                                                            {{ $passportProvienceList->publishStatus == '0' ? 'selected' : '' }}>
                                                                            Unpublished</option>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="POST" id="deleteForm">
                    ...
                    @method('DELETE')
                    <div class="modal-body">
                        Are you sure you want to delete this Provience ?
                    </div>
                    <div class="modal-footer">
    ...
    <script>
        ...
        function setDeleteFormAction(id) {
            document.getElementById('deleteForm').action = '{{ route('passportProvienceList.destroy', '') }}/' + id;
        }
    </script>
    ...
    <script>
        ...
        const publishUrl = @json(route('passportProvienceList.publish', ['id' => '__ID__']));
        const unpublishUrl = @json(route('passportProvienceList.unpublish', ['id' => '__ID__']));
