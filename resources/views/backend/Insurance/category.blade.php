...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="content-wrapper">
            <h4 class="fw-bold mb-4"><span class="text-muted fw-light"> </span>Country</h4>
            <!-- Main Content -->
            <div class="row">
                ...
                <div class="col-12 col-lg-8">
                    <div class="card mb-4">
                        <div class="card-header d-flex align-items-center justify-content-between">
                            <h5 class="mb-0">{{ isset($visaCountryList) ? 'Edit' : 'Add' }} Country Lists</h5>
                        </div>
                        <div class="card-body">
                            <form method="POST"
                                action="{{ isset($visaCountryList)
                                    ? route('visaCountryList.update', ['visaCountryList' => $visaCountryList->id])
                                    : route('visaCountryList.store') }}"
                                id="JobForm">
                                @csrf
                                @if (isset($visaCountryList))
                                    @method('PUT')
                                @endif
                        ...
                        <div class="card-body">
                            <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($visaCountryLists as $visaCountryList)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $visaCountryList->countryName }}</td>
                                                <td>
                                                    <p
                                                        class="text-capitalize  badge {{ $visaCountryList->publishStatus == '1' ? 'bg-success' : 'bg-danger' }} m-2">
                                                        {{ $visaCountryList->publishStatus == '1' ? 'Published' : 'Unpublished' }}
                                                    </p>
                                                </td>
                                                <td>
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu">
                                                            ...
                                                            <a class="dropdown-item text-{{ $visaCountryList->publishStatus == '1' ? 'danger' : 'success' }}"
                                                                href="javascript:void(0);" data-bs-toggle="modal"
                                                                data-bs-target="#publishUnpublishModal"
                                                                onclick="setPublishUnpublishFormAction({{ $visaCountryList->id }}, '{{ $visaCountryList->publishStatus }}', '{{ $visaCountryList->visaCountryListName }}')">
                                                                <i
                                                                    class="bx bx-{{ $visaCountryList->publishStatus == '1' ? 'x' : 'check' }} me-1"></i>
                                                                {{ $visaCountryList->publishStatus == '1' ? 'Unpublish' : 'Publish' }}
                                                            </a>

                                                            <!-- Edit Trigger -->
                                                            <a class="dropdown-item text-primary" href="#"
                                                                data-bs-toggle="modal"
                                                                data-bs-target="#editvisaCountryListModal{{ $visaCountryList->id }}">
                                                                <i class="bx bx-edit me-1"></i> Edit
                                                            </a>
                                                            <!-- Delete Trigger -->
                                                            <a class="dropdown-item text-danger" href="javascript:void(0);"
                                                                data-bs-toggle="modal" data-bs-target="#deleteModal"
                                                                onclick="setDeleteFormAction({{ $visaCountryList->id }})">
                                            ...
                                            <div class="modal fade" id="editvisaCountryListModal{{ $visaCountryList->id }}"
                                                tabindex="-1" aria-labelledby="editvisaCountryListModalLabel"
                                                aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <form id="editvisaCountryListForm"
                                                            action="{{ route('visaCountryList.update', ['visaCountryList' => $visaCountryList->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('PUT')
                                                            ...
                                                            <div class="modal-body">
                                                                <div class="mb-3">
                                                                    ...
                                                                    <input type="text" class="form-control"
                                                                        ...
                                                                        value="{{ $visaCountryList->countryName }}"
                                                                        required>
                                                                </div>
                                                                <div class="mb-3">
                                                                    ...
                                                                    <select class="form-select"
                                                                        ...
                                                                        required>
                                                                        <option value="1"
                                                                            {{ $visaCountryList->publishStatus == '1' ? 'selected' : '' }}>
                                                                            Published</option>
                                                                        <option value="0"
                                                                            {{ $visaCountryList->publishStatus == '0' ? 'selected' : '' }}>
                                                                            Unpublished</option>
    ...
    <script>
        ...
        function setDeleteFormAction(id) {
            document.getElementById('deleteForm').action = '{{ route('visaCountryList.destroy', '') }}/' + id;
        }
    </script>
    ...
    <script>
        ...
        const publishUrl = @json(route('visaCountryList.publish', ['id' => '__ID__']));
        const unpublishUrl = @json(route('visaCountryList.unpublish', ['id' => '__ID__']));
